...
class LinearFeatureBaseline(nn.Module):
    ...
    def _feature(self, episodes):
        ...
        time_step = torch.arange(len(episodes)).view(-1, 1, 1).to(self.weight.device)*ones / 100.0
        return torch.cat([
            observations,
            observations ** 2,
            time_step,
    ...
    def fit(self, episodes):
        ...
        for _ in range(5):
            try:
                coeffs, _ = torch.lstsq(XT_y, XT_X + reg_coeff * self._eye.to(self.weight.device))
