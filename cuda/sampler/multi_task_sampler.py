...
from sampler.sampler import Sampler, make_env
from env.utils.sync_vector_env import SyncVectorEnv
from utils.episode import BatchEpisodes
...
class MultiTaskSampler(Sampler):
    ...
    """
    def __init__(self, config, env_name, env_kwargs, batch_size, adapt_lr, policy, baseline, env=None, seed=None):
        ...
        # self.tr_task_queue = Queue()
        # self.vl_task_queue = Queue()
    ...
    def sample_tasks(self, num_tasks, **kwargs):
        tasks = self.env.unwrapped.sample_tasks(num_tasks)
        tasks = [(index, task, kwargs) for index, task in enumerate(tasks)]
        self.task_list = tasks
            # self.tr_task_queue.put()
            # self.vl_task_queue.put((index, task, kwargs))
        return tasks
    ...
    def adapt(self, train_episodes, first_order=None):
        ...
        adaptedParams = []
        for epi in train_episodes :
            params = None
            inner_loss = reinforce_loss(self.config.device, self.policy, epi, params=params)
            params = self.policy.update_params(inner_loss,
                                               params=params,
                                               step_size=self.adapt_lr,
                                               first_order=first_order)
            adaptedParams.append(params)
    ...
    def sample_episodes(self, tasks, **kwargs):
        gae_lambda = kwargs.get('gae_lambda')

        train_episodes = []
        for task in tasks :
            params = None
            gamma = task[2].get('gamma')
            device = task[2].get('device')
            episodes = BatchEpisodes(batch_size=self.batch_size, gamma=gamma, device=device)
            episodes.log('_createdAt', datetime.now(timezone.utc))

            t0 = time.time()
            for item in self.sample_trajectories(task, params=params):
            ...
            train_episodes.append(episodes)

        updated_params = self.adapt(train_episodes, first_order=self.config.first_order)

        valid_episodes = []
        for task, param in zip(tasks, updated_params) :
            gamma = task[2].get('gamma')
            device = task[2].get('device')
            episodes = BatchEpisodes(batch_size=self.batch_size, gamma=gamma, device=device)
            episodes.log('_createdAt', datetime.now(timezone.utc))
            ...
            t0 = time.time()
            for item in self.sample_trajectories(task, params=param):
                episodes.append(*item)
            episodes.log('duration', time.time() - t0)

            self.baseline.fit(episodes)
            episodes.compute_advantages(self.baseline, gae_lambda=gae_lambda, normalize=True)
            valid_episodes.append(episodes)

        return (train_episodes, valid_episodes, updated_params)

    def sample_trajectories(self, task, params):
        batch_id = task[0]
        kwargs = task[1]
        self.env.reset_task(kwargs)
        observation = self.env.reset()
        observations = np.repeat(np.expand_dims(observation,0),self.config.fast_batch_size,0) 
        envs = [deepcopy(self.env) for _ in range(self.config.fast_batch_size)]

        dones = np.array([False] * self.config.fast_batch_size)
        with torch.no_grad():
            while not dones.all():
                observations_tensor = torch.from_numpy(observations).to(self.config.device)
                pi = self.policy(observations_tensor, params=params)
                actions_tensor = pi.sample()
                actions = actions_tensor.cpu().numpy()
                new_obs_list, rewards_list, batch_ids = [], [], []
                for idx, (env, action) in enumerate(zip(envs, actions)) :
                    new_observation, reward, done, info = env.step(action)
                    if dones[idx] :
                        new_observation = observations[idx]
                        reward = 0
                        done = True
                    new_obs_list.append(new_observation)
                ...
                observations = np.array(new_obs_list)
...
class SamplerWorker(mp.Process):
    def __init__(self, index, env_name, env_kwargs, batch_size, observation_space, action_space,
                 policy, baseline, seed, task_queue, train_queue, valid_queue, policy_lock):
