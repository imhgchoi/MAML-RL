...
def weighted_mean(device, tensor, lengths=None):
    if lengths is None:
    ...
    lengths = torch.as_tensor(lengths, dtype=torch.float32).to(device)
...
def weighted_normalize(device, tensor, lengths=None, epsilon=1e-8):
    mean = weighted_mean(device, tensor, lengths=lengths)
    out = tensor - mean.mean()
    for i, length in enumerate(lengths):
        out[length:, i].fill_(0.)

    std = torch.sqrt(weighted_mean(device, out ** 2, lengths=lengths).mean())
...
def vector_to_parameters(vector, parameters):
    ...
    for param in parameters:
        ...
        params.append(vector[pointer:pointer + num_param].view_as(param).data)
