...
from utils.torch_utils import weighted_normalize
...
class BatchEpisodes(object):
    ...
    def returns(self):
        if self._returns is None:
            ...
            return_ = torch.zeros((self.batch_size,), dtype=torch.float32).to(self.device)
    ...
    def compute_advantages(self, baseline, gae_lambda=1.0, normalize=True):
        ...
        gae = torch.zeros((self.batch_size,), dtype=torch.float32).to(self.device)
        for i in range(len(self) - 1, -1, -1):
        ...
        if normalize:
            self._advantages = weighted_normalize(self.device, self._advantages,
